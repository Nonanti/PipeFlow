...
namespace PipeFlow.Tests
{
    public class CloudTests
    {
        ...
        [Fact]
        public async Task PipeFlow_S3Csv_CreatesValidPipeline()
        {
            // This would need actual S3 credentials to work
            // For now, we're just testing that the method exists
            Assert.NotNull(PipeFlow.Core.PipeFlow.From);
            
            // Method signature test
            var methodInfo = typeof(PipeFlow.Core.PipeFlow.PipeFlowBuilder)
                .GetMethod("S3Csv");
            Assert.NotNull(methodInfo);
        }

        [Fact]
        public async Task PipeFlow_AzureBlobCsv_CreatesValidPipeline()
        {
            // Method signature test
            var methodInfo = typeof(PipeFlow.Core.PipeFlow.PipeFlowBuilder)
        ...
        [Fact]
        public async Task PipeFlow_GoogleCloudCsv_CreatesValidPipeline()
        {
            // Method signature test
            var methodInfo = typeof(PipeFlow.Core.PipeFlow.PipeFlowBuilder)
